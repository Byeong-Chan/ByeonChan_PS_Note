...
struct Geometry {
    ...
    point o[MAXN]; // point (hull or set of points)
    // pointf of[MAXN];
    point convex[MAXN];
    // pointf convexf[MAXN];
    ...
    long double get_area(point *hull) { // need a hull
        long double ans = 0;
        for(int i=0;i<N;i++) {
    ...
    long double get_areaf(pointf *hull) { // need a hull
        long double ans = 0;
        for(int i=0;i<N;i++) {
            int a = i, b = i + 1 == N ? 0 : i + 1;
            ans += hull[a].x * hull[b].y - hull[a].y * hull[b].x;
...
int main() {
    int n = 3;
    dec.init(n);
    printf("%d\n",dec.ccw(dec.o[0], dec.o[1], dec.o[2]));
    return 0;
}