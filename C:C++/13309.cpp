    ...
    void decomposition() {
        ...
        while(q.size()) {
            ...
            if (path[u] > 1 && sw) {
                first[segcnt] = 1;
                last[segcnt] = path[u];
                segtree[segcnt] = make_node();
                seg_init(1, path[u], segtree[segcnt]);
                ...
                while(path[tmp]) {
                    HLDup[tmp] = goal;
                    dec_pos[tmp] = segcnt + 1;
                    seg_update(1, path[u], path[tmp], mothvalue[tmp], segtree[segcnt]);
                    tmp = rep_moth[0][tmp];
                }
                segcnt++;
            }
...
int main() {
    ...
    for(i=0;i<Q;i++) {
        scanf("%d %d %d",&x,&y,&z);
        if (dec.HLD_Query(x, y) == 1) {
            printf("YES\n");
            if (z == 1) dec.edge_update(x, dec.rep_moth[0][x], 2);
        }
        else {
            printf("NO\n");
            if (z == 1) dec.edge_update(y, dec.rep_moth[0][y], 2);
        }
    }
    return 0;
}