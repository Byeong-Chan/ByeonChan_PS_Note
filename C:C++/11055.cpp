...
struct nodes {
    ...
    int pos;
    nodes() {}
    nodes(int ihap, int ival, int ipos) {
        hap = ihap;
        val = ival;
        pos = ipos;
    }
    bool operator > (const nodes &a) const {
        if(hap != a.hap) return hap > a.hap;
...
void init(int n) {
    for(k=1;k<n;k<<=1);
    for(int i = k << 1; i > 0; i--) segtree[i] = nodes(0, 0, 0);
}
...
int o[1010];
int dx[1010];
nodes dp1[1010], dp2[1010];

int main() {
    int i, n;
    int ex = 0;
    int tmp;
    nodes here, ans(0, 0, 0x7fffffff);
    scanf("%d",&n);
    for(i=1;i<=n;i++) {
        scanf("%d",&o[i]);
        dx[i-1] = o[i];
